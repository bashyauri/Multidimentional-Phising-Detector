...
for name, model in models.items():
    ...
    if name in ["RandomForest", "XGBoost", "HistGradientBoosting", "MLP"]:
        search = RandomizedSearchCV(model, param_grids[name], n_iter=15, cv=3, scoring='f1', n_jobs=-1, random_state=42)
    else:
        search = GridSearchCV(model, param_grids[name], cv=3, scoring='f1', n_jobs=-1)
