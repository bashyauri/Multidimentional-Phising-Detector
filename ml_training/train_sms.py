...
def train_sms_model(dataset_path: Path):
    ...
    metrics["model"] = "LogisticRegression + TF-IDF (balanced, 70/15/15 split)"
    ...
    model_out = MODELS_DIR / "sms_model_validation.pkl"
    joblib.dump(pipeline, model_out)

    save_confusion_plot(metrics["confusion_matrix"], "sms_validation")
    write_metrics("sms_validation", metrics)

    print("SMS model trained successfully with three-way split")
